...
WeakClassifier* AdaBooster::get_best_classifier(){
	...
	for (unsigned int i=0; i<dimensions; i++){//50
		...
		// traverse features
		for (unsigned int j=0; j<num_features; j++){//200

			// if we correctly classified the point, decrement tmp_pos_err
			if (sorted[i][j]->label() == POS)
	...
	float am = weight_classifier(lowest_err);	
  	float threshold = sorted[dimension][index]->at(dimension);
	//printf("dimension: %d, index: %d, at(dimension): %f, at(index):%f", dimension, index, sorted[dimension][index]->at(dimension), sorted[dimension][index]->at(index));
	FeatureVector* fc = sorted[dimension][index];
	
	// create weak classifier pointer and return
	WeakClassifier* wc = new WeakClassifier(dimension, threshold, is_flipped, am, index, lowest_err);

	////////////////////////////////////////////
	cout << endl;
	//sorted[dimension][index]->printFeature();
	
	//cout << "accuracy: " << 1 - lowest_err << endl;
	//cout << "position : " << dimension << endl;//(선택된 feature에서 어디 부분인지->몇번째 feature==>picture 인지)
	//cout << "feature_index : " << index << endl;//(선택된 feature 의 고유 number)-> 내 feature==>picture에서 몇번째인지feature)
	//cout << "threshold: " << threshold << endl;//sorted[dimension][index]->at(dimension) : 선택된 feature에서 해당 위치에 해당하는 값(featue값)
	
	//11-10 weak classifier에 feature 넣음
	wc->setFeature(*sorted[dimension][index]);
	FeatureVector tmp = *sorted[dimension][index];
	vector<Element> ev=tmp.getFvec();
	...
	//FeatureVector f = *sorted[dimension][index];
	/*cout << "weight_1:"<< (*sorted[dimension][index]).weight() << endl;
	cout << "weight_2:" << weight << endl;*/
	//////////////////////////////////
